...
class WeatherAvailability: ObservableObject {
    ...
    public func availabilityCheck(for works: [Criteria], with weatherList: [Weather]) -> [String: [String]] {
        ...
        for weather in weatherList {
            ...
            for work in works {
                // 날씨 조건 확인
                let isTemperatureOkay = Double(weather.temperature) ?? 4 >= work.temperaturemin && Double(weather.temperature) ?? 4 <= work.temperaturemax
                let isRainOkay = Double(weather.rain) ?? 0 <= work.rainfall
                let isSnowOkay = Double(weather.snow) ?? 0 <= work.snow
                let isWindOkay = Double(weather.windspeed) ?? 0 <= work.wind
